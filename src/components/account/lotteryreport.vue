<template>
  <div class="lotteryreport">
    <Row>
      ...
      <Col span="6">
      <p class="title">日期:</p>
      <DatePicker :clearable="false" @on-change="search()" type="date" :options="options1" v-model="startTime" placeholder="请选择日期" style="width: 118px"></DatePicker>
      <span style="margin: 5px;">-</span>
      <DatePicker :clearable="false" @on-change="search()" type="date" :options="options2" v-model="endTime" placeholder="请选择日期" style="width: 118px"></DatePicker>
      </Col>
      <Col span="5">
    ...
    <div class="table">
      <div class="table-title">
        <div>当前查询账号：{{crumb?Account:userDetail.account}} 上次更新时间：{{newTime}} &nbsp;&nbsp;&nbsp;&nbsp;代理报表查询时间为 起始时间07:00 ~ 截止时间07:00 的数据。例如：2018-01-01 07:00 ~ 2018-01-02 07:00
          <div>
...
<script>
  ...
  export default {
      ...
      return {
        ...
        columns1: [{
          ...
          },
          {
            ...
            key: 'total_pay',
            render: (h, params) => {
              if (params.row.member_account.indexOf('总计') === -1 && params.row.total_pay !== 0) {
                return h('span', {
                  style: {
          ...
          },
          {
            ...
            key: 'p_preferential',
            render: (h, params) => {
              if (params.row.member_account.indexOf('总计') === -1 && params.row.p_preferential !== 0) {
                return h('span', {
                  style: {
                    color: 'rgb(16, 142, 233)',
                    cursor: 'pointer'
    ...
    computed: { ...mapState("account", ['queryLotteryReportList']),
      ...mapState(['userDetail', 'Times'])
    },
    methods: {
      ...mapActions("account", [
        "queryLotteryReport"
        ...
        this.queryLotteryReport(data).then(res => {
          ...
          if (res.code === 0) {
            ...
            if (that.dataList.length !== 0) {
              ...
              that.dataList.push({
                'member_account': '总计' + that.dataList.length + '人',
                'cnt': cnt,
                'has_child': has_child,
                'p_deposit': p_deposit,
                'p_draw': p_draw,
                'p_preferential': p_preferential,
...
<style>
  ...
  .lotteryreport .ivu-breadcrumb {
    display: flex !important;
  }
  .lotteryreport .ivu-breadcrumb span:first-child {
    display: flex !important;
  }
</style>
