<template>
	<div class="spreadlink">
		<Form :label-width="286" :model="formLeft">
			...
			<FormItem label="注册码：">
				<input type="text" disabled="disabled" class="spreadlink_zc_code" v-model='queryPromoteUrlList.invitationCode'>
				<Button type="primary" style="margin-left: 20px;" v-clipboard:copy=this.queryPromoteUrlList.invitationCode v-clipboard:success="onCopy" v-clipboard:error="onError">复制 </Button>
			</FormItem>
			<FormItem label="是否有代理功能：">
				<RadioGroup v-model="formLeft.radio">
					<Radio label="1" name="sf" checked="checked">是</Radio>
					<Radio label="0" name="sf">否</Radio>
				</RadioGroup>
			</FormItem>
			<FormItem label="下级模式：">
				<RadioGroup v-model="formLeft.mode">
					<Radio label="A" name="sf">A</Radio>
					<Radio label="B" name="sf">B</Radio>
					<Radio label="C" name="sf">C</Radio>
					<Radio label="D" name="sf">D</Radio>
				</RadioGroup>
			</FormItem>
		...
		methods: {
			...
			PromoteUrl() {
				this.queryPromoteUrl().then((res) => {
					...
					this.formLeft.mode = res.data.childModel;
					this.formLeft.radio = res.data.childHierarchy
