<template>
  <div class="chase" style="padding:8px ;">
    <RadioGroup v-model="confirmBetType">
      <span>下注方式：</span>
      <Radio label="1">普通投注</Radio>
      <Radio v-if="classId!='1'&&classId!='2'" label="2" :disabled="disabled1"><span :style="disabled1?{color:'rgba(0,0,0,.25)'}:''">常规追号</span></Radio>
      <Radio v-if="classId!='1'&&classId!='2'" label="3" :disabled="disabled2"><span :style="disabled2?{color:'rgba(0,0,0,.25)'}:''">高级追号</span></Radio>
    </RadioGroup>
    <div class="explain">
    ...
    <div class="setChase">
      <div class="box_1" v-show="confirmBetType=='2'">
        ...
        <Row style="margin-top: 4px;">
          <Col span="6">
          <span>追号期数</span>：
          <InputNumber :max="10000" :min="1" v-model="chasePeriods"></InputNumber>
          </Col>
          <Col span="6">
          <span>起始倍数</span>：
          <InputNumber :max="10000" :min="1" v-model="chaseTimes"></InputNumber>
          </Col>
        </Row>
      </div>
      <div class="box_2" v-show="confirmBetType=='3'">
        ...
        <Row style="margin-top: 4px;">
          ...
          <Col span="6">
          <span>已投入(元)</span>：
          <InputNumber :max="10000" :min="1" v-model="chasePrePay"></InputNumber>
          </Col>
        </Row>
        <Row>
          <RadioGroup v-model="chaseWinType" vertical>
            <Radio label="1" style="margin: 10px 0;">
              <span>全程最低盈利率(%):</span>
              <InputNumber :max="10000" :min="1" v-model="chaseMinPercent"></InputNumber>
            </Radio>
            <Radio label="2" style="margin: 10px 0;">
              <span>前:</span>
              <InputNumber :max="10000" :min="1" style="width: 70px;" v-model="chaseBeforePeriod"></InputNumber><span style="margin-right: 12px;margin-left: 2px;">期</span>
              <span>盈利率(%):</span>
              <InputNumber :max="10000" :min="1" v-model="chaseBeforePercent"></InputNumber>
              <span style="margin-left: 20px;">之后盈利率(%):</span>
              <InputNumber :max="10000" :min="1" style="width: 70px;" v-model='chaseAfterPercent'></InputNumber>
            </Radio>
            <Radio label="3" style="margin: 10px 0;">
              <span>全程最低盈利(元):</span>
              <InputNumber :max="10000" :min="1" style="width: 70px;" v-model="chaseMinNumber"></InputNumber>
            </Radio>
          </RadioGroup>
        </Row>
      </div>
  ...
  export default {
        ...
        columns: [{
            ...
            render: (h, params) => {
              ...
              if (params.index == 0 && this.currentPage == 1) {
                ...
                return h("div", params.row.periodNo);
              }
            }
          },
          {
                ...
                on: {
                  "on-change": value => {
                    ...
                    this.onchangeChase(this.cachingArray).then(() => {
                      this.tempArray = this.cachingArray.slice(
          ...
          },
          {
            ...
            render: (h, params) => {
              params.row = this.tempArray[params.index];
              return h("div", params.row.currentPay);
            }
          },
          {
            title: "累积投入(元)",
            ...
            render: (h, params) => {
              params.row = this.tempArray[params.index];
              return h("div", params.row.hasPay);
            }
          },
          {
            title: "盈利(元)",
            ...
            render: (h, params) => {
              ...
              return h("div", params.row.profit);
            }
          },
          {
            title: "盈利率",
            ...
            render: (h, params) => {
              params.row = this.tempArray[params.index];
              return h("div", params.row.profitRate);
            }
          }
        ]
      };
    ...
    methods: {
      ...
      async goChase() {
        ...
        console.log(this.subGameList[0]);
        ...
        if (this.chasePeriods > this.chaseList.length) {
          this.$Message.error(
            "您设定的条件只能生成" + this.chaseList.length + "期追号"
          );
        } else {
          this.$Message.success("生成追号方案成功");
        }
        this.setChasingNoteList({
          cachingArray: this.cachingArray,
