<template>
	<div class="betPlatte">
		<div v-if="selectMode == '1'&&showCheckBox" style="display: flex;align-items: center;flex-direction: column;margin-top: 10px;">
			<div>温馨提示：你选择了
				<Tag checkable color="#d2eafb">{{checkAllGroup.length}}</Tag>个位置，系统自动根据位置组合成
				<Tag checkable color="#d2eafb">{{selectNum}}</Tag>个方案。
				<Checkbox :indeterminate="indeterminate" :value="checkAll" style="margin: 0 20px" @click.prevent.native="handleCheckAll">全选</Checkbox>
			</div>
			...
			<div v-for="(item1,index1) in betDataList.numbers" :key="index1">
				...
				<Row class="hotCold" v-if="showHotColdOrOmit.indexOf(subGameId)!=-1&&fruit.indexOf('冷热')!=-1">
					...
					<Row>
						...
						<Col span="20">
						<Tag v-if="item.count!='null'" v-for="(item,index) in hotCold[index1+1+'']" :key="index" :class="item.hotCold !=null?item.hotCold == 'blue'?'bgblue':'bgred':''">{{item.count}}</Tag>
						</Col>
					</Row>
					</Col>
				</Row>
				<Row class="omit" v-if="showHotColdOrOmit.indexOf(subGameId)!=-1&&fruit.indexOf('遗漏')!=-1">
					...
					<Row>
						...
						<Col span="20">
						<Tag v-if="item.count!='null'" v-for="(item,index) in omit[index1+1+'']" :key="index" :class="item.hotCold !=null?item.hotCold == 'blue'?'bgblue':'bgred':''">{{item.count}}</Tag>
						</Col>
					</Row>
					</Col>
				</Row>
			...
			<div v-for="(item1,index1) in betDataList.numbers" :key="index1">
				<div class="btn_num">
					<div class="btn_box" v-for="(item2,index2) in item1.cols" :key="index2">
						...
						<div class="odd" :key="index">{{subGameList[index2].oddMax}}</div>
					</div>
				</div>

				<!--<div class="btn_num reversal">
					<div class="btn_box" v-for="(item2,index2) in item1.cols" :key="index2" v-if="index2>(item1.cols.length)/2-1">
							<Button :style="item2.data_selected?selectedStyle:''" @click="onSelect({row:index1,col:index2})">{{item2.label}}</Button>
					</div>
				</div>-->
	...
	export default {
		...
		data() {
			return {
				...
				squareButtonArr: ["300312", "300412", "300512", "400201", "400301", "400901", "400501", "200201", "200701", "200705", "200709", "200713", "200717", "200721", "200502"],
		...
		methods: {
			...
			judgeFromat() {
				...
				if(this.selectMode == '1' && this.showCheckBox) {
					result = calcInputBetCount(this.betDataList, this.inputTxt, this.checkAllGroup);
				} else {
			...
			insererNotes() {
				...
				data.periodNo = this.awarPeriod;
				if(boolen) {
					let lotteryData = bet_fn.getLotteryData(data, this.checkAllGroup, this.selectMode);
			...
			quickBets() {
				...
				data.rightName = this.rightName;
				data.subGameId = this.subGameId;
				data.totalPrice = Math.round(this.totalPrice * 100) / 100;
				data.price = this.price;
				data.unite = this.unite;
				data.subGameList = this.subGameList;
				data.kindId = this.kindId;
				...
				if(boolen) {
					...
					let lotteryData = bet_fn.getLotteryData(data, this.checkAllGroup, this.selectMode);
					let params = bet_fn.quickBets({
						lotteryData: lotteryData,
						kindId: this.kindId,
						orderFrom: "1",
						orderType: "0",
						periodNo: this.awarPeriod
					});
...
<style>
	...
	.betPlatte .sscSpecial .ivu-col-span-12 .ivu-row,
	.betPlatte .sscSpecial .ivu-col-span-12 .ivu-row {
		display: flex;
		align-items: center;
	}
	...
	.betPlatte .sscSpecial .ivu-col-span-12 .ivu-row .ivu-col-span-4,
	.betPlatte .sscSpecial .ivu-col-span-12 .ivu-row .ivu-col-span-20 {
		display: flex;
	}
	...
	.betPlatte .sscSpecial .ivu-col-span-9 .ivu-row .ivu-col-span-4,
	.betPlatte .sscSpecial .ivu-col-span-9 .ivu-row .ivu-col-span-20 {
		display: flex;
		align-items: center;
	}
	...
	.betPlatte .hotCold .ivu-tag.bgblue,
	.betPlatte .omit .ivu-tag.bgblue {
		background: #d2eafb;
		border: none;
	}
	
	.betPlatte .hotCold .ivu-tag.bgred,
	.betPlatte .omit .ivu-tag.bgred {
		background: #fcdbd9;
		border: none;
	}
	
	.betPlatte .hotCold .ivu-tag.bgblue span,
	.betPlatte .omit .ivu-tag.bgblue span {
		color: #108ee9;
	}
	
	.betPlatte .hotCold .ivu-tag.bgred span,
	.betPlatte .omit .ivu-tag.bgred span {
		color: #f04134;
	}
	...
	.betPlatte .hotCold .ivu-tag,
	.betPlatte .omit .ivu-tag {
		width: 38px;
		height: 22px;
		text-align: center;
		background: none;
