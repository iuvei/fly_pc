	...
	<div class="bet_menu" v-if="show">
		<div v-if="selectMode!=2">
			<Menu mode="horizontal" theme="primary" :active-name="0" class="level_1" ref="level_1" @on-select="changeIndex1">
				<MenuItem :name="index" :key="index" v-for="(item1,index) in standGameList" v-if="item1.gameId!='nn'"> {{item1.gameName}}
				</MenuItem>
			</Menu>
			<!--二级导航-->
			<Menu mode="horizontal" class="level_2" @on-select="changeIndex2" ref="level_2" :active-name="activeName">
				<MenuItem :name="item.leftName+'-'+item.rightName+'-'+item.subGameId+'-'+index" :key="index" v-for="(item,index) in standGameList[lv1_idx].rightList" v-if="standGameList[lv1_idx].showRight" class="item"> {{item.gameId=='4001'||item.gameId=='2003'?item.leftName:item.rightName}}
				</MenuItem>
				<MenuItem :name="val[0].leftName+'-'+val[0].rightName+'-'+val[0].subGameId+'-'+index" :key="index" v-for="(val, key,index) of standGameList[lv1_idx].gameSons" v-if="val.length == 1&&!standGameList[lv1_idx].showRight" class="item"> {{key}}
				</MenuItem>
		...
		<div v-else>
			<Menu mode="horizontal" theme="primary" :active-name="0" class="level_1" ref="level_1" @on-select="onSelectSubGame">
				<MenuItem :name="item1.showId" :key="index" v-for="(item1,index) in menu_list[classId]">{{item1.title}}
				</MenuItem>
			</Menu>
			<BetPlatteLmp :dataList="dataList_lmp" :classId="classId" :showId="showId" :kindId="kindId"></BetPlatteLmp>
		</div>
...
<script>
	...
	import BetPlatteLmp from "./lmp_betPlatte.vue";
	...
	export default {
		...
		created() {
			...
			this.onSelectSubGame(this.showId)
		},
		methods: {
			getGamePlayList(params) {
				this.gamePlay(params).then(() => {
					...
					this.show = true;
					// this.$nextTick(() => {
					// 	console.log(`$nextTick_____create`)
			...
			onSelectSubGame(name) {
				this.showId = name;
				this.dataList_lmp = createNmubers_lmp(this.classId, this.showId)
			}
		},
		computed: {
...
<style>
	...
	.bet_menu .level_2 .ivu-menu-item.item,
	.bet_menu .level_2 .ivu-menu-submenu {
	...
	.bet_menu .ivu-menu-submenu,
	.bet_menu .ivu-menu-item.item {
		padding-left: 15px;
		padding-right: 15px;
	}
