<template>
  <div id="report">
    <!-- 一个模态框开始 -->
    <Modal v-model="detail" title="注单明细" width="700" class-name="detail">
      <Form ref="formInline">
        ...
        <FormItem label="下注号码：" :label-width="125">
          <Row>
            <Col span="24">
              <textarea v-model="dataDetail.lotteryNumbers" readonly></textarea>
            </Col>
          </Row>
    ...
    <div class="report_head">
      ...
      <div>
        ...
        <Col span="12" style="float: left;z-index: 999">
          <DatePicker @on-change="selectTime" type="date" placeholder="开始时间" :clearable="false" style="width: 130px" transfer :options="{disabledDate:disableStartTime}" v-model="startTime"></DatePicker>
        </Col>
        <!--<div style="width: 10px;height: 32px;background-color: #CCCCCC;float: left"></div>-->
        <Col span="12" style="float: left">
          <DatePicker @on-change="selectTime" type="date" placeholder="结束时间" :clearable="false" style="width: 130px" transfer :options="{disabledDate:disableEndTime}" v-model="endTime"></DatePicker>
        </Col>
      </div>
      <div>
        <div>快速选择:</div>
    ...
    <div class="report_table" v-show="sels">
      ...
      <div class="report_table_but">
        <div>
          当前查看账号：{{account}}，更新时间：{{Time(new Date())}} {{ nowTime}}
          <Button v-show="account!=userDetail.account" @click="queryLoginAccount" type="ghost">查看当前登录账号</Button>
        </div>
      ...
      <div class="go_router" v-if="this.userDetail.type == 0">
        <Button @click="returnLotteryReporter" type="primary" style="float: right">返回到代理报表</Button>
      </div>
    </div>
    <!-- 第一张表结束 -->
    ...
    <div class="report_tableA" v-show="sel">
      ...
      <div>
        ...
        <ul class="tableZ" v-show="showZ">
          <li style="width: 388px">
            <div style="text-align: right;">小计</div>
          </li>
          <li>
            <div>{{datas.data.sumMin.orderAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMin.rebateAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMin.winAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMin.backAmount.toFixed(2)}}</div>
          </li>
          <li style="width: 340px;border-right: 1px solid #e9eaec;">
            <div :style="{color:datas.data.sumMin.realAmount>0?'red':datas.data.sumMin.realAmount<0?'#21c447':'#495060'}">{{datas.data.sumMin.realAmount.toFixed(2)}}</div>
          </li>
        </ul>
        <ul class="tableZ" v-show="showZ">
          <li style="width: 388px">
          ...
          <li>
            <div>{{datas.data.sumMax.orderAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMax.rebateAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMax.winAmount.toFixed(2)}}</div>
          </li>
          <li>
            <div>{{datas.data.sumMax.backAmount.toFixed(2)}}</div>
          </li>
          <li style="width: 340px;border-right: 1px solid #e9eaec;">
            <div :style="{color:datas.data.sumMax.realAmount>0?'red':datas.data.sumMax.realAmount<0?'#21c447':'#495060'}">{{datas.data.sumMax.realAmount.toFixed(2)}}</div>
          </li>
        </ul>
      </div>
      <div class="go_router">
...
<script>
  ...
  export default {
        ...
        this.memberIfBind().then(res => {
          if (res.code == '1') {
            ...
            new Promise((resolve, reject) => {
              ...
              that.$store.commit('transmitTimes', {
                mark: 1,
                account: that.account,
                startTime: that.startTime,
                endTime: that.endTime
    ...
    computed: mapState(["isLogin","kindList", "userDetail", "MemberReport"]),
    watch: {
      userDetail() {
        this.personReport()
      }
