<template>
  <div class="account" v-if="show">
    <Menu ref="contactMenu" mode="horizontal" :active-name="activeName" @on-select="changeRouter">
      <MenuItem name="0">登入记录</MenuItem>
      <MenuItem name="1">我的银行卡</MenuItem>
      <MenuItem name="2">存取记录</MenuItem>
      <MenuItem name="3">代理报表</MenuItem>
      <MenuItem name="4">账变记录</MenuItem>
      <!--<MenuItem name="10"> 分红契约
	      </MenuItem>
	      <MenuItem name="9"> 日工资契约
      </MenuItem>-->
      <MenuItem name="5">用户安全</MenuItem>
      <MenuItem name="6">团队资料</MenuItem>
      <MenuItem name="7">团队转点</MenuItem>
      <MenuItem name="8" v-if="userDetail.hierarchy!=0">推广链接</MenuItem>
      <MenuItem name="9">赚水设置</MenuItem>
      <MenuItem name="10" v-if="userDetail.hierarchy != 1">联系上级</MenuItem>
      <MenuItem name="11">下级充提</MenuItem>
    </Menu>
...
export default {
  ...
  methods: {
    ...mapActions("account", ["memberIfBind", "saveMemberBank"]),
    routingReset() {
