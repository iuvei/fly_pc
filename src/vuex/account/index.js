...
export default {
  ...
  mutations: {
    ...
    changeMemberModel(state, params) {
      state.memberModel[params.index][params.model] = params.data
    },
    updateOwnInfo(state, data) {
      state.updateOwnInfoList = data
    },
    queryOrderByNo(state, data) {
      state.queryOrderByNo = data
    }
  },
  actions: {
    //查询登入记录
    ...
    // 查询注单号
    async queryOrderByNo({ commit }, data) {
      let res = await fetch('post', 'queryOrderByNo', data)
      commit('queryOrderByNo', res)
      return res
    },
    //代理会员
    async queryAgentMemberInfo({
      commit
    }, data) {
    ...
    // 转为代理
    async updateMemberAgent({
      commit
    }, data) {
      let res = await fetch('post', 'updateMemberAgent', data);
      commit('updateMemberAgent', res);
      return res
    },
    // 推广链接1
    async queryPromoteUrl({
      commit
    }, data) {
    ...
    // 联系上级
    async touchSuper() {
      let res = await fetch('get', 'touchSuper')
      return res
    },
    // 注册
    ...
    async queryOwnRebate({
      commit
    }) {
      let res = await fetch('get', 'queryOwnRebate');
    ...
    async updateOwnInfo({
      ...
      let res = await fetch('post', 'updateOwnInfo', params);
      commit("updateOwnInfoList", res)
      return res
    },
    async queryMemberModel({
      commit
    }, params) {
      let res = await fetch('get', 'queryMemberModel', params);
    ...
    async updateMemberModel({
      ...
      let res = await fetch('post', 'updateMemberModel', params);
      // commit("updateMemberModel", res)
      return res
    },
    // 转点下级
    async superToSub({ commit }, params) {
      let res = await fetch('post', 'superToSub', params);
      return res
    },
    // 非一级代理申请转点
    async applyPoint({ commit }, params) {
      let res = await fetch('post', 'applyPoint', params);
      return res
    },
    // 上级处理下级申请转点
    async queryApplyPoint() {
      let res = await fetch('get', 'queryApplyPoint')
      return res
    },
    async disposeApplyPoint({ commit }, params) {
      let res = await fetch('post', 'disposeApplyPoint', params);
      return res
    }
  }
}
