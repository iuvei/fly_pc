...
function calcChooseBetCount(betNumbers, optionalFields) {
	//	console.log(betNumbers, optionalFields)
...
function calcInputBetCount(betNumbers, inputNumber, optionalFields) {
...
function calcProfitAndRate(_a) {
	var odds = _a.odds,//赔率
		chaseTimes = _a.chaseTimes,//倍数
		betTimes = _a.betTimes,
		hasPay = _a.hasPay;//累计投入
		console.log(odds,chaseTimes,betTimes,hasPay)
...
function createBetItem(betNumbers, betType, betRate, optionalFields, ticketCost, ticketCount, unitPrice) {
	var numberType = betNumbers.numberType,
		subGameId = betNumbers.subGameId,
		numbers = betNumbers.numbers,
		optionalMin = betNumbers.optionalMin;
	var oddMax = betRate.oddMax,
		oneNoteLimit = betRate.oneNoteLimit,
		singleLimit = betRate.singleLimit;
	var leftName = betType.leftName,
		rightName = betType.rightName;
	var numberString = '',
		mapIndex = 0,
		needSeperatorInCol = false;
	if(numberType === "button") {
		var rows_1 = [];
		if(numbers.length === 1) {
			needSeperatorInCol = true
		}
		numbers.forEach(function(item) {
			var colString = [];
			item.cols.forEach(function(subItem) {
				if(subItem.data_selected) {
			...
			rows_1.push(colString.join(needSeperatorInCol ? ',' : ''))
		});
		numberString = rows_1.join()
	} else {
	...
	var betArray = [];
	var oddArray = oddMax.split(',');
	var betName = rightName && leftName !== rightName ? leftName + "(" + rightName + ")" : leftName;
	if(subGameId === "300312" || subGameId === "300412" || subGameId === "300512") {
		numbers[0].cols.forEach(function(item, index) {
			if(item.data_selected) {
				betArray.push({
					betName: betName,
...
const blueLabels = ['03', '04', '09', '10', '14', '15', '20', '25', '26', '31', '36', '37', '41', '42', '47', '48'];
const redLabels = ['01', '02', '07', '08', '12', '13', '18', '19', '23', '24', '29', '30', '34', '35', '40', '45', '46'];
const greenLabels = ['05', '06', '11', '16', '17', '21', '22', '27', '28', '32', '33', '38', '39', '43', '44', '49'];
var num_red = {},
	num_blue = {},
	num_green = {};
redLabels.forEach((item) => {
	num_red[item] = '#ff2e44';
})
blueLabels.forEach((item) => {
	num_blue[item] = '#03a9f4';
})
greenLabels.forEach((item) => {
	num_green[item] = '#87de75';
})
...
function createBetNumbers(betType) {
	var leftName = betType.leftName,
		rightName = betType.rightName,
...
function createSpecialBetItem(_a) {
	var betNumbers = _a.betNumbers,
		unitPrice = _a.unitPrice;
	var betArray = [];
	var numbers = betNumbers.numbers,
		firstSubGameId = betNumbers.subGameId;
	if(createSpecialBetNumbers.dtSumMap[firstSubGameId]) {
		var sumColString = [];
		numbers.forEach(function(row) {
			var rowName = row.rowName,
				...
				oneNoteLimit = row.oneNoteLimit,
				singleLimit = row.singleLimit;
			var betName = rightName && leftName !== rightName ? leftName + "(" + rightName + ")" : leftName;
			var colString = [];
			cols.forEach(function(col, index) {
				var selected = col.data_selected,
					label = col.label;
				if(selected) {
					if(index === 2) {
		...
		if(sumColString.length > 0) {
			var _b = numbers[numbers.length - 1],
				rowName = _b.rowName,
				subGameId = _b.subGameId,
				cols = _b.cols,
				oddMax = _b.oddMax,
				leftName = _b.leftName,
				rightName = _b.rightName,
				oneNoteLimit = _b.oneNoteLimit,
				singleLimit = _b.singleLimit;
			var orderNum = sumColString.length;
			var betName = rightName && leftName !== rightName ? leftName + "(" + rightName + ")" : leftName;
			betArray.push({
				betName: betName,
				bonus: oddMax,
				...
				singleLimit: singleLimit,
				subGameId: subGameId,
				totalAmount: unitPrice * orderNum,
				unitPrice: unitPrice
			})
		}
	} else if(createSpecialBetNumbers.sumOEMap[firstSubGameId] || createSpecialBetNumbers.zhengheMap[firstSubGameId] || BJSCBetNumbersConfig.special[firstSubGameId]) {
		numbers.forEach(function(row) {
			var rowName = row.rowName,
				subGameId = row.subGameId,
				cols = row.cols,
		...
		numbers.forEach(function(row) {
			...
			cols.forEach(function(col) {
				var selected = col.selected,
					label = col.label,
					oddMax = col.oddMax,
					leftName = col.leftName,
					rightName = col.rightName,
					oneNoteLimit = col.oneNoteLimit,
					singleLimit = col.singleLimit,
					subGameId = col.subGameId;
				var betName = rightName && leftName !== rightName ? leftName + "(" + rightName + ")" : leftName;
				if(selected) {
					betArray.push({
						betName: betName,
...
function loopNumbers(_a) {
	var betRates = _a.betRates,
		betTypeMap = _a.betTypeMap,
		labels = _a.labels,
		subGameId = _a.subGameId;
	var numbers = [];
	...
	return __assign({}, betRate, betType, {
		cols: numbers,
...
function createSpecialBetNumbers(props) {
	// console.log(props)
	var betRates = props.betRates,
		betTypeMap = props.betTypeMap,
	...
	subGameIds.forEach(function(subGameId) {
		...
		if(KSBetNumbersConfig.special[subGameId]) {
			...
			numbers.push(__assign({}, betRate, betType, {
...
function normalChooseNumber(_a) {
	var betNumbers = _a.betNumbers,
		rowIndex = _a.rowIndex,
		colIndex = _a.colIndex;
	var newBetNumbers = __assign({}, betNumbers);
	var subGameId = betNumbers.subGameId,
		dantuoMax = betNumbers.dantuoMax;
	var selectedNumber = newBetNumbers.numbers[rowIndex].cols[colIndex];
	var beforeStatus = selectedNumber.data_selected;
	selectedNumber.selectedTime = Date.now();
	if(SSCBetNumbersConfig.fullInverse[subGameId]) {
		newBetNumbers.numbers[rowIndex].cols.forEach(function(item) {
			return item.data_selected = false
		})
	}
	selectedNumber.data_selected = !beforeStatus;
	if(calcBetCountConfig.dantuo[subGameId]) {
		if(selectedNumber.data_selected) {
			newBetNumbers.numbers[1 - rowIndex].cols[colIndex].data_selected = false;
			if(rowIndex === 0) {
				var oldestSelected_1 = Date.now() * 10,
					oldestIndex_1 = -1,
					selecteds_1 = 0;
				newBetNumbers.numbers[rowIndex].cols.forEach(function(item, index) {
					if(item.data_selected) {
						selecteds_1 += 1
					}
					if(item.selectedTime && item.selectedTime < oldestSelected_1) {
						oldestSelected_1 = item.selectedTime;
						oldestIndex_1 = index
					}
				});
				if(selecteds_1 === dantuoMax) {
					newBetNumbers.numbers[0].cols[oldestIndex_1].data_selected = false;
					newBetNumbers.numbers[0].cols[oldestIndex_1].selectedTime = undefined
				}
			}
		}
	}
	if(SSCList[subGameId] && subGameId.substr(2, 2) === '14' && selectedNumber.inverse !== undefined) {
		var inverseNumber = newBetNumbers.numbers[rowIndex].cols[selectedNumber.inverse];
		if(selectedNumber.data_selected && inverseNumber) {
			inverseNumber.data_selected = false
		}
	}
...
function numberToChinese(money) {
	...
	var str = '';
	var tempArray = money.toString().split('.');
	var intPart = tempArray[0].split('');
	var decimalPart = [];
	if(tempArray[1]) {
	...
	intPart.forEach(function(item, index) {
		var intItem = parseInt(item);
		var realIndex = (intPartLength - index) % 4;
		if(intItem > 0) {
			if(hasZeroBefore) {
				str += numbers[0];
				hasZeroBefore = false
	...
	if(decimalPartLength > 0) {
		decimalPart.forEach(function(item, index) {
			var intItem = parseInt(item);
			if(intItem > 0) {
				str += numbers[intItem];
...
function getRandomInt(plus, add) {
	if(add === void 0) {
		add = 0
	}
...
function combineTimesClac(numbers, n) {
	return combination(numbers, n).reduce(function(prev, item) {
		return prev + item.reduce(function(subPrev, subItem) {
			return subPrev * subItem.length
		}, 1)
	}, 0)
};

function optionalFourClac(firstRow, secondRow, n) {
	var ticketCount = 0;
